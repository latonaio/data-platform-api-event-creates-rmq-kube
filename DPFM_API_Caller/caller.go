...
import (
	...
	"data-platform-api-event-creates-rmq-kube/config"
...
func (c *DPFMAPICaller) subfuncProcess(
	...
	for _, fn := range accepter {
		...
		case "Campaign":
			c.campaignCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		case "Game":
			c.gameCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		case "PointTransaction":
			c.pointTransactionCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		case "PointConditionElement":
			c.pointConditionElementCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		default:
			wg.Done()
		}
	}
...
func (c *DPFMAPICaller) campaignCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	//}()
	//defer wg.Done()
	//err = c.complementer.ComplementCampaign(input, subfuncSDC, log)
	//if err != nil {
	//	mtx.Lock()
	//	*errs = append(*errs, err)
	//	mtx.Unlock()
...
func (c *DPFMAPICaller) gameCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	//}()
	//defer wg.Done()
	//err = c.complementer.ComplementGame(input, subfuncSDC, log)
	//if err != nil {
	//	mtx.Lock()
	//	*errs = append(*errs, err)
	//	mtx.Unlock()
...
func (c *DPFMAPICaller) pointTransactionCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	//}()
	//defer wg.Done()
	//err = c.complementer.ComplementPointTransaction(input, subfuncSDC, log)
	//if err != nil {
	//	mtx.Lock()
	//	*errs = append(*errs, err)
	//	mtx.Unlock()
...
func (c *DPFMAPICaller) pointConditionElementCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	//}()
	//defer wg.Done()
	//err = c.complementer.ComplementPointConditionElement(input, subfuncSDC, log)
	//if err != nil {
	//	mtx.Lock()
	//	*errs = append(*errs, err)
	//	mtx.Unlock()
